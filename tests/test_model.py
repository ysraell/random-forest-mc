...
def create_mock_tree(survived_score=0.5, data=None, class_vals=None, features=None, used_features=None):
    ...
    mock_tree.used_features = used_features if used_features is not None else ["feature1"]
    mock_tree.md5hexdigest = "dummy_hash"
    mock_tree.__call__.return_value = {"classA": 1.0, "classB": 0.0} # Default return for tree call
    return mock_tree
...
class TestRandomForestMC:
    def test_init(self):
        ...
        assert rf.max_depth is not None # Should be set to recursion limit or int(max_depth)
    ...
    def test_process_dataset(self):
        ...
        with patch('src.random_forest_mc.model.log') as mock_log:
            ...
            mock_log.warning.assert_called_with("Temporal features ordering disable: you do not have all orderable features!")
    ...
    def test_sampleFeats(self):
        ...
        rf_temporal = RandomForestMC(min_feature=1, max_feature=3, temporal_features=True)
        rf_temporal.target_col = "target"
        temporal_feature_cols = ["feat_1", "feat_3", "feat_2", "target"]
        with patch('numpy.random.randint', return_value=2):
            with patch('numpy.random.sample', return_value=np.array(["feat_3", "feat_1"])):
                sampled_feats_temporal = rf_temporal.sampleFeats(temporal_feature_cols.copy())
                assert sampled_feats_temporal == ["feat_1", "feat_3"] # Should be sorted
    ...
    def test_splitData_numeric(self):
        ...
        df = pd.DataFrame({"feat1": [1, 2, 3, 4, 5], "target": ["a", "b", "a", "b", "a"]})
        ...
        df_edge = pd.DataFrame({"feat1": [1, 1, 1, 1, 1], "target": ["a", "b", "a", "b", "a"]})
    ...
    def test_splitData_categorical(self):
        rf = RandomForestMC(target_col="target")
        rf.numeric_cols = []
        df = pd.DataFrame({"feat1": ["red", "blue", "red", "green"], "target": ["a", "b", "a", "b"]})
    ...
    def test_plantTree(self, MockDecisionTreeMC):
        ...
        with patch.object(rf, 'genLeaf', return_value={"leaf": {"A": 1.0}, "depth": "2#"}):
            with patch.object(rf, 'splitData', return_value=(df_train.iloc[:2], df_train.iloc[2:], 2.5)):
    ...
    def test_validationTree(self):
        ...
        mock_tree.__call__.side_effect = [{"A": 0.9, "B": 0.1}, {"A": 0.2, "B": 0.8}] # Predict A, then B
        ...
        mock_tree.__call__.side_effect = [{"A": 0.9, "B": 0.1}, {"A": 0.9, "B": 0.1}] # Predict A, then A
    ...
    def test_survivedTree(self, mock_validationTree, mock_plantTree, mock_sampleFeats, mock_split_train_val):
        rf = RandomForestMC(th_start=0.8, delta_th=0.1, max_discard_trees=3, get_best_tree=True)
        ...
        rf_best = RandomForestMC(th_start=0.8, delta_th=0.1, max_discard_trees=1, get_best_tree=True)
        ...
        assert mock_validationTree.call_count == 3 # Called for each tree + one more for the final check

        # Scenario 3: Tree fails, threshold decreases
        rf_th_decease = RandomForestMC(th_start=0.8, delta_th=0.1, max_discard_trees=1, get_best_tree=False)
    ...
    def test_sampleClassFeatCount(self):
        ...
        rf.class_vals = ["A", "B"]
        rf.data = [
            create_mock_tree(survived_score=0.8, class_vals=["A", "B"], used_features=["f1", "f2"]),
            create_mock_tree(survived_score=0.7, class_vals=["A", "B"], used_features=["f1"]),
        ]
        rf.data[0].__call__.return_value = {"A": 0.9, "B": 0.1} # Predicts A
        rf.data[1].__call__.return_value = {"A": 0.1, "B": 0.9} # Predicts B
    ...
    def test_sampleClassFeatPairImportance(self):
        ...
        rf.data = [
            create_mock_tree(survived_score=0.8, class_vals=["A", "B"], used_features=["f1", "f2"]),
            create_mock_tree(survived_score=0.7, class_vals=["A", "B"], used_features=["f1", "f3"]),
        ]
        rf.data[0].__call__.return_value = {"A": 0.9, "B": 0.1} # Predicts A
        rf.data[1].__call__.return_value = {"A": 0.1, "B": 0.9} # Predicts B
    ...
    def test_validationMissingValues(self):
        ...
        with patch('src.random_forest_mc.model.log') as mock_log:
            ...
            mock_log.warning.assert_called_with("The Forest model have not the following feature(s): [f4].")
    ...
    def test_genFilledDataMissing(self, mock_fill_row_missing):
        ...
        mock_fill_row_missing.side_effect = [None, None] # All rows have no missing values
    ...
    def test_predictMissingValues(self, mock_predict_proba, mock_genFilledDataMissing, mock_validationMissingValues):
        rf = RandomForestMC()
        ...
        mock_genFilledDataMissing.return_value = (original_data, filled_data.drop(columns=["_target_prob_classA", "_target_prob_classB"]))
...
def test_DictValuesAllFeaturesMissing_exception():
    ...
    assert "All features in the given dictionary 'dict_values' are not found" in str(excinfo.value)
